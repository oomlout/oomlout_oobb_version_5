...
def main():
    ...
    for root, _, files in os.walk(base):
        for f in files:
            ...
            # check json/yaml pairs generated by run_case
            if f == 'item.yaml':
                rel = os.path.relpath(os.path.join(root, f), base)
                yaml_new = os.path.join(current, rel)
                json_new = yaml_new.replace('item.yaml', 'item.json')
                if os.path.exists(yaml_new) and os.path.exists(json_new):
                    with open(yaml_new) as fy, open(json_new) as fj:
                        y_data = yaml.safe_load(fy)
                        j_data = json.load(fj)
                    if y_data != j_data:
                        with open(report_file, 'a') as report:
                            report.write(f'YAML/JSON mismatch: {rel}\n')
                        ok = False
