...
def describe():
    global d
    d = {}
    d["name"] = "withering_hbbd_logo"
    d["name_long"] = "Withering HBBD Logo"
    d["description"] = (
        "Rounded-square HBBD checkerboard badge with the black SVG regions "
        "subtracted from the top face for engraving or colour inlay."
    )
    d["category"] = "OOBB Custom Components"
    d["shape_aliases"] = ["withering_hbbd_logo", "withuering_hbbd_logo"]
    ...
    v.append(
        _variable(
            "logo_size_percent",
            "Logo width as a fraction of the badge side length.",
            "number",
...
def _copy_logo_file(source_path, cache_dir):
    ...
    destination_name = "withering_hbbd_logo.svg"
...
def render(params):
    ...
    if not os.path.isfile(logo_file):
        raise FileNotFoundError(f"withering_hbbd_logo SVG not found: {logo_file}")
    ...
    # half a unit before scaling so the checkerboard is centred on the body.
